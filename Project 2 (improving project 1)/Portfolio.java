...
public class Portfolio {
    ...
    public void updatePrices() {
        ...
        String type;
        do {
            System.out.print("Enter type to update (stock/mutualfund): ");
            type = scanner.nextLine().toLowerCase();
        } while (!type.equals("stock") && !type.equals("mutualfund") && !type.equals("s") && !type.equals("m"));

        if (type.equals("s")) type = "stock";
        if (type.equals("m")) type = "mutualfund";

        System.out.print("Enter symbol of the " + type + " to update: ");
        String symbol = scanner.nextLine();

        Investment investment = findInvestment(symbol);
        if (investment != null && 
            ((type.equals("stock") && investment instanceof Stock) || 
             (type.equals("mutualfund") && investment instanceof MutualFund))) {
            System.out.print("Enter new price for " + type + " with symbol " + investment.getSymbol() + ": ");
            double newPrice = scanner.nextDouble();
            investment.setPrice(newPrice);
            System.out.println("Price updated for " + type + " with symbol " + investment.getSymbol() + ".");
        } else {
            System.out.println("No matching investment found with symbol " + symbol + " in " + type + ".");
        }
    }
...
public double getGain() {
    ...
    for (Investment investment : investments) {
        boolean matchesType = (type.equals("s") && investment instanceof Stock) ||
                              (type.equals("m") && investment instanceof MutualFund);
        boolean matchesSymbol = investment.getSymbol().equalsIgnoreCase(symbol);

        if (matchesType && matchesSymbol) {
     ...
     */
    public void search(String symbol, String keyword, double lowPrice, double highPrice) {
        ...
        if (!resultPositions.isEmpty()) {
            for (int position : resultPositions) {
                System.out.println(investments.get(position));
            }
        } else {
            System.out.println("No investments found with the given criteria.");
        }
    }
